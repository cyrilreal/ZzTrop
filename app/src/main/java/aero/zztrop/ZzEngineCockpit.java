...
public class ZzEngineCockpit {
    ...
    public void computeRest() {
        ...
        if (pattern.equals("R / x / x")) {
            ...
            calTempEnd.setTimeInMillis(calTempStart.getTimeInMillis() + restShort.getTimeInMillis());
            alTimePairs.add(new TimePair(calTempStart, calTempEnd, PERIOD_TYPE_REST));

            // 2nd time pair calculation
            calTempStart.setTimeInMillis(calTempEnd.getTimeInMillis());
            calTempStart.add(Calendar.MINUTE, pause);

            calTempEnd.setTimeInMillis(calTempStart.getTimeInMillis() + xTime);
            alTimePairs.add(new TimePair(calTempStart, calTempEnd, PERIOD_TYPE_REST));
        ...
        // compute short rest duration and round it to the
        // lower minute by zeroing seconds
        if (pattern.equals("x / x / R")) {

            restShort.setTimeInMillis(amp.getTimeInMillis() - convertMinutesToMillis(pause * 2) - (xTime * 2));
            restShort.set(Calendar.SECOND, 0);
